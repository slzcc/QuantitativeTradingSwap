            ...
            if self.side != '多':
                ...
                # 当锚点为负数时, 证明已下过单
                    ...
                    ## 第一单盈利大于 0.00566 左右就可以盈利清仓
                        ...
                        res_long = trade.open_order(self.symbol, 'BUY', sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)][-2:]), price=round(float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))) * (1 - self.min_profit), self.price_precision), positionSide='SHORT').json()
                        self.redisClient.setKey('{}_orderId_{}'.format(self.token, res_long["orderId"]), json.dumps(res_long))
                        res_long = trade.open_order(self.symbol, 'BUY', sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)][:-2]), price=round(float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))) * (1 - self.profit), self.price_precision), positionSide='SHORT').json()
                        self.redisClient.setKey('{}_orderId_{}'.format(self.token, res_long["orderId"]), json.dumps(res_long))
                    ...
                    # 判断 第一次开仓后 && (当前价格 小于等于 购买价格 * (1 - self.min_profit)) && 最低价格 < 100000
                        ...
                        if float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) >= float(self.redisClient.getKey("{}_lowest_price_{}".format(self.token, self.direction))) * (1 + (1 - float(self.redisClient.getKey("{}_lowest_price_{}".format(self.token, self.direction))) / float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction)))) / 5):  # 重仓情形考虑回本平一半或平xx%的仓位, 待计算, 剩下依然重仓考虑吃多少点清仓
                            ...
                            if float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) <= float(self.redisClient.getKey("{}_base_price_{}".format(self.token, self.direction))) * (1 - self.profit):
                                ...
                                # _env["CalculatedValue"] = float(self.redisClient.getKeys("{}_last_sell_{}".format(self.token, self.direction))) * (1 + self.add_rate * np.log(1 - int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction)))))
                                self.logger.info(_env)
                                res_short = trade.open_order(self.symbol, 'SELL', [float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)][0], price=float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), positionSide='SHORT').json()
                                self.redisClient.setKey('{}_orderId_{}'.format(self.token, res_short["orderId"]), json.dumps(res_short))
                                ...
                                self.redisClient.lpushKey("{}_buy_qty".format(self.token), self.redisClient.getKey("{}_position_size_{}".format(self.token, self.direction)))
                                _win = ([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)][-1] * float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) * 4e-4) - float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction)))
                                self.redisClient.setKey("{}_win_{}".format(self.token, self.direction), _win)
                        ...
                        # 下单平仓
                        res_long = trade.open_order(self.symbol, 'BUY', [float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)][-1], price=float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), positionSide='SHORT').json()
                        self.redisClient.setKey('{}_orderId_{}'.format(self.token, res_long["orderId"]), json.dumps(res_long))
            ...
            # 如果策略为开 多 时
                ...
                # 判断起始位大于 0, 至少开过一次仓
                    ...
                    if self.if_loss and (not condition) and float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) <= float(self.redisClient.getKey("{}_last_buy_{}".format(self.token, self.direction))) * (1 - self.add_rate * np.log(1 + int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction))))):
                        ...
                        _env["condition"] = condition
                        # _env["CalculatedValue"] = float(self.redisClient.getKeys("{}_last_buy_{}".format(self.token, self.direction))) * (1 + self.add_rate * np.log(1 - int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction)))))
                        self.logger.info(_env)
