...
def to_log(msg):
    with open('logs/QuantitativeTradingSwap.log','a+') as f:
        f.write(msg+'\n')
...
class GridStrategy:
    ...
    def grid_run(self):
        ...
        time.sleep(1)
        t_start = time.time()
        to_log('%s U本位开始运行 \t%s\t#################' % (self.symbol, PublicModels.changeTime(time.time())))
        while True:
            # 获取 币 1m 时间段的 k 线
            ...
            #     t_start = time.time()
            to_log('{}/{} U本位合约正在运行, 当前价格 {} \t {}'.format(self.symbol, self.side,self.present_price, PublicModels.changeTime(time.time())))
            # 记录最小购买单价
            ...
            if self.side != '多':
                ...
                if self.step == 0:
                    ...
                    if sell_condition1 or sell_condition2:
                        ...
                        # 下单成功
                        to_log('%s 当前仓位成本=%.1f，开仓价=%.3f\t%s' % (self.symbol, sum(self.buy_qty)*self.avg, self.avg, PublicModels.changeTime(time.time())))
                ...
                # 起始位非 0
                    ...
                    if self.if_loss and (not condition) and self.present_price >= self.last_sell * (1 + self.add_rate * np.log(1 - self.step)):
                        ...
                        if self.present_price <= self.highest_price*(1-(self.highest_price/self.last_sell-1)/5):
                            ...
                            if not 'orderId' in res_short:
                                if res_short['msg'] == 'Margin is insufficient.':
                                    to_log('{} 可用金不足 \t {} \t {}'.format(self.symbol, str(res_short), PublicModels.changeTime(time.time())))
                                else:
                                    to_log('{} 加仓失败 \t {} \t {}'.format(self.symbol, str(res_short), PublicModels.changeTime(time.time())))
                                continue
                            ...
                            to_log('%s 当前仓位成本=%.1f，均价=%.3f，浮亏=%.2f，已实现盈利=%.2f（最大持有量=%s,%.1f小时）\t%s' % (
                        ...
                        # 给本轮马丁挂上止盈，重置重新开始下一轮
                        to_log('{} 重新开始下一轮'.format(self.symbol))
                    ...
                    # 判断 第一次开仓后 && (当前价格 小于等于 购买价格 * (1 - ))
                        ...
                        if self.present_price >= self.lowest_price * (1 + (1 - self.lowest_price / self.avg) / 5):
                            to_log('{} 平空'.format(self.symbol))
                            res_long = trade.open_order(self.symbol, 'BUY', sum(self.buy_qty), price=None, positionSide='SHORT')
                            if not 'orderId' in res_long:
                                to_log('%s 平空失败 \t%s\t%s' % (self.symbol, str(res_long), PublicModels.changeTime(time.time())))
                                continue
                            self.win += sum(self.buy_qty) * (self.avg - self.present_price) * (1-4e-4)
                            self.step = 0
                            self.avg = 0.0
                            ...
                            self.base_price = 0.0
                            self.avg_tmp = 0.0
                            to_log('%s 清仓，已实现盈利=%.2f（最大持有量=%s,%.1f小时）\t%s' % (self.symbol, self.win, self.max_position, (time.time() - self.t_start) / 3600, PublicModels.changeTime(time.time())))
                            ...
                            if self.present_price <= self.base_price * (1 - self.profit):
                                ...
                                if not 'orderId' in res_short:
                                    if res_short['msg'] == 'Margin is insufficient.':
                                        to_log('%s 可用金不足\t%s\t%s' % (self.symbol, str(res_short), PublicModels.changeTime(time.time())))
                                    else:
                                        to_log('%s 加仓失败\t%s\t%s'%(self.symbol, str(res_short), PublicModels.changeTime(time.time())))
                                    continue
                                self.base_price *= 1 - self.profit
                                self.buy_qty.append(self.buy_qty[0])
                                self.win -= self.buy_qty[-1] * self.present_price * 4e-4
                                to_log('%s 当前仓位成本=%.1f，均价=%.3f，浮盈=%.2f，已实现盈利=%.2f（最大持有量=%s,%.1f小时）\t%s' % (
                        ...
                        to_log('{} 平最近一次加仓'.format(self.symbol))
                        # 下单平仓
                        ...
                        to_log('%s 剩余仓位成本=%.1f，均价=%.3f，浮盈=%.2f，已实现盈利=%.2f（最大持有量=%s,%.1f小时）\t%s' % (self.symbol, sum(self.buy_qty)*self.avg, self.avg, sum(self.buy_qty) * (self.avg - self.present_price), self.win, self.max_position, (time.time() - self.t_start) / 3600, PublicModels.changeTime(time.time())))
                time.sleep(6)
            ...
            # 如果策略为开 多 时
                ...
                if self.step == 0:
                    ...
                    if buy_condition1 or buy_condition2:
                        to_log('{} 开多'.format(self.symbol))
                        # 下单开多
                        res_long = trade.open_order(self.symbol, 'BUY', self.position_size, price=None, positionSide='LONG')
                        # 判断是否下单成功
                        if not 'orderId' in res_long:
                            to_log('{} 开多失败 \t {} \t {}'.format(self.symbol, str(res_long), PublicModels.changeTime(time.time())))
                            time.sleep(10)
                            continue
                        self.avg = self.present_price
                        self.base_price = self.avg
                        ...
                        self.win -= self.position_size * self.present_price * 4e-4
                        # 开单成功后
                        to_log('%s 当前仓位成本=%.1f，开仓价=%.3f\t%s' % (self.symbol, sum(self.sell_qty) * self.avg, self.avg, PublicModels.changeTime(time.time())))
                ...
                # 判断起始位大于 0, 至少开过一次仓
                    ...
                    if self.if_loss and (not condition) and self.present_price <= self.last_buy * (1 - self.add_rate * np.log(1 + self.step)):
                        to_log('%s 平多止损'.format(self.symbol))
                        res_short = trade.open_order(self.symbol, 'SELL', sum(self.sell_qty), price=None, positionSide='LONG')
                        if not 'orderId' in res_short:
                            to_log('%s 平多失败 \t%s\t%s' % (self.symbol, str(res_short), PublicModels.changeTime(time.time())))
                            continue
                        ...
                        if self.present_price >= self.lowest_price * (1 + (1 - self.lowest_price / self.last_buy) / 5):
                            ...
                            if not 'orderId' in res_long:
                                if res_long['msg'] == 'Margin is insufficient.':
                                    to_log('%s 可用金不足 \t%s\t%s' % (self.symbol, str(res_long), PublicModels.changeTime(time.time())))
                                else:
                                    to_log('%s 加仓失败 \t%s\t%s'%(self.symbol, str(res_long), PublicModels.changeTime(time.time())))
                                continue
                            self.avg = (self.avg + self.present_price) / 2
                            self.last_buy = self.present_price
                            ...
                            to_log('%s 当前仓位成本=%.1f，均价=%.3f，浮亏=%.2f，已实现盈利=%.2f（最大持有量=%s,%.1f小时）\t%s' % (self.symbol, sum(self.sell_qty) * self.avg, self.avg,
                        ...
                        if self.present_price <= self.highest_price * (1 - (self.highest_price / self.avg - 1) / 5):  # 重仓情形考虑回本平一半或平xx%的仓位，待计算，剩下依然重仓考虑吃多少点清仓
                            ...
                            if self.present_price>=self.base_price * (1 + self.profit):
                                ...
                                if not 'orderId' in res_long:
                                    if res_long['msg'] == 'Margin is insufficient.':
                                        to_log('%s可用金不足\t%s\t%s' % (self.symbol, str(res_long), PublicModels.changeTime(time.time())))
                                    else:
                                        to_log('%s加仓失败\t%s\t%s'%(self.symbol, str(res_long), PublicModels.changeTime(time.time())))
                                    continue
                                self.base_price *= 1 + self.profit
                        ...
                        to_log('%s 剩余仓位成本=%.1f，均价=%.3f，浮盈=%.2f，已实现盈利=%.2f（最大持有量=%s,%.1f小时）\t%s' % (self.symbol, sum(self.sell_qty)*self.avg, self.avg, sum(self.sell_qty) * (self.present_price - self.avg), self.win, self.max_position, (time.time() - self.t_start) / 3600, PublicModels.changeTime(time.time())))
