            ...
            for keyName in keyList:
                ...
                if orderInfo["status"] == "FILLED":
                    ...
                    if orderInfo["side"] == "BUY":
                        if direction == "LONG":
                            redisClient.lpushKey("{}_real_sell_qty".format(token), orderInfo["origQty"])
                        else:
                            redisClient.lpushKey("{}_real_buy_qty".format(token), orderInfo["origQty"])

                    elif orderInfo["side"] == "SELL":
                        if direction == "LONG":
                            redisClient.brpopKey("{}_real_sell_qty".format(token))
                        else:
                            redisClient.brpopKey("{}_real_buy_qty".format(token))
...
class GridStrategy(Process):
    def __init__(self, symbol, key, secret, token):
        ...
        # last_buy default
        # 开多时的购买价格
        if not self.redisClient.getKey("{}_last_buy_{}".format(self.token, self.direction)):
            self.redisClient.setKey("{}_last_buy_{}".format(self.token, self.direction), 0.0)
        # last_sell default
        # 开空时的购买价格
        if not self.redisClient.getKey("{}_last_sell_{}".format(self.token, self.direction)):
            self.redisClient.setKey("{}_last_sell_{}".format(self.token, self.direction), 0.0)
        ...
        while True:
            ...
            if self.side != '多':
                ...
                if int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction))) == 0:
                    ...
                    if sell_condition1 and sell_condition2:
                        ...
                        # 记录购买数量到 buy_qty 列表中
                        self.redisClient.lpushKey("{}_buy_qty".format(self.token), self.redisClient.getKey("{}_position_size_{}".format(self.token, self.direction)))
                        # 记录出售价格
                        self.redisClient.setKey("{}_last_sell_{}".format(self.token, self.direction), self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction)))
                        ...
                        self.logger.info('%s/%s 当前仓位成本=%.1f, 开仓价=%.3f \t %s' % (
                            self.symbol,
                            self.side,
                            sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]) * float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))),
                ...
                # 当锚点为负数时, 证明已下过单
                    ...
                    # 判断是否可以继续下单，返回布尔值
                    # 开仓总币价 / 每单币价 < 开仓数量
                    condition = sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]) / self.min_qty < self.max_add_times
                    # 判断 仓位 是否需要进行止损(全仓平仓)
                    ## 判断 亏损 && (是否可以继续开仓) && 当前价格 大于等于 准备出售价格 乘以 (1 + 1.2 * 开仓数量比例值) ep: 19700.0 * (1 + 1.2 * np.log(1 - -1))
                    ## 主要判断亏损如果超过范围则进行止损平仓（开仓数量到达上限）
                    if self.if_loss and (not condition) and float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) >= float(self.redisClient.getKey("{}_last_sell_{}".format(self.token, self.direction))) * (1 + self.add_rate * np.log(1 - int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction))))):
                        ...
                        _env["CalculatedValue"] = float(self.redisClient.getKeys("{}_last_sell_{}".format(self.token, self.direction))) * (1 + self.add_rate * np.log(1 - int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction)))))
                        self.logger.info(_env)

                        # 下单平空(市价平所有仓位)
                        res_long = trade.open_order(self.symbol, 'BUY', sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]), price=float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), positionSide='SHORT').json()
                        self.redisClient.setKey('{}_orderId_{}'.format(self.token, res_long["orderId"]), json.dumps(res_long))
                        # 判断下单平空
                        ...
                        self.redisClient.setKey("{}_base_price_{}".format(self.token, self.direction), 0.0)
                        self.redisClient.setKey("{}_avg_tmp_{}".format(self.token, self.direction), 0.0)
                        self.redisClient.delKey("{}_buy_qty".format(self.token))

                    ## 如果仓位亏损继续扩大则到达比例后进行加仓
                    ## 判断是否可以加仓
                    elif condition and float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) >= float(self.redisClient.getKey("{}_last_sell_{}".format(self.token, self.direction))) * (1 + self.add_rate * np.log(1 - int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction))))):
                        self.redisClient.setKey("{}_highest_price_{}".format(self.token, self.direction)), max(float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), float(self.redisClient.getKey("{}_highest_price_{}".format(self.token, self.direction))))
                        if float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) <= float(self.redisClient.getKey("{}_highest_price_{}".format(self.token, self.direction))) * (1 - (float(self.redisClient.getKey("{}_highest_price_{}".format(self.token, self.direction))) / float(self.redisClient.getKey("{}_last_sell_{}".format(self.token, self.direction))) - 1) / 5):
                            ...
                            # 下单加仓
                            res_short = trade.open_order(self.symbol, 'SELL', sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]), price=float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), positionSide='SHORT').json()
                            self.redisClient.setKey('{}_orderId_{}'.format(self.token, res_short["orderId"]), json.dumps(res_short))
                            # 判断下单加仓
                            if not 'orderId' in res_short:
                            ...
                            self.redisClient.setKey("{}_last_buy_{}".format(self.token, self.direction), float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))))
                            self.redisClient.lpushKey("{}_buy_qty".format(self.token), sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]))
                            _win = ([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)][-1] * float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) * 4e-4) - float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction)))
                            ...
                            self.logger.info('%s/%s 当前仓位成本=%.1f, 均价=%.3f, 浮亏=%.2f, 已实现盈利=%.2f（最大持有量=%s, %.1f小时）\t%s' % (
                                ...
                                sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]) * float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))), 
                                float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))), 
                                sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]) * (float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))) - float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction)))), 
                    ...
                    ## 第一单盈利大于 0.00566 左右就可以盈利清仓
                    elif (not condition) and float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) >= float(self.redisClient.getKey("{}_last_sell_{}".format(self.token, self.direction))) * (1 + self.add_rate * np.log(1 - int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction))))):
                        ...
                        res_long = trade.open_order(self.symbol, 'BUY', sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)][-2:]), price=round(self.avg * (1 - self.min_profit), self.price_precision), positionSide='SHORT').json()
                        self.redisClient.setKey('{}_orderId_{}'.format(self.token, res_long["orderId"]), json.dumps(res_long))
                        res_long = trade.open_order(self.symbol, 'BUY', sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)][:-2]), price=round(self.avg * (1 - self.profit), self.price_precision), positionSide='SHORT').json()
                        self.redisClient.setKey('{}_orderId_{}'.format(self.token, res_long["orderId"]), json.dumps(res_long))
                    ...
                    # 判断 第一次开仓后 && (当前价格 小于等于 购买价格 * (1 - self.min_profit)) && 最低价格 < 100000
                        ...
                        if float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) >= float(self.redisClient.getKey("{}_lowest_price_{}".format(self.token, self.direction))) * (1 + (1 - float(self.redisClient.getKey("{}_lowest_price_{}".format(self.token, self.direction))) / float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction)))) / 5):  # 重仓情形考虑回本平一半或平xx%的仓位, 待计算, 剩下依然重仓考虑吃多少点清仓
                            ...
                            if float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) <= float(self.redisClient.getKey("{}_base_price_{}".format(self.token, self.direction))) * (1 - self.profit):
                                ...
                                _env["CalculatedValue"] = float(self.redisClient.getKeys("{}_last_sell_{}".format(self.token, self.direction))) * (1 + self.add_rate * np.log(1 - int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction)))))
                                self.logger.info(_env)
                                res_short = trade.open_order(self.symbol, 'SELL', [float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)][0], price=float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), positionSide='SHORT').json()
                                self.redisClient.setKey('{}_orderId_{}'.format(self.token, res_short["orderId"]), json.dumps(res_short))
                                ...
                                _base_price = (1 - self.profit) * self.redisClient.getKey("{}_base_price_{}".format(self.token, self.direction))
                                self.redisClient.setKey("{}_base_price_{}".format(self.token, self.direction), _base_price)
                                self.redisClient.lpush("{}_buy_qty".format(self.token), self.redisClient.getKey("{}_position_size_{}".format(self.token, self.direction)))
                                _win = (self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)[-1] * self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction)) * 4e-4) - float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction)))
                                self.redisClient.setKey("{}_win_{}".format(self.token, self.direction), _win)

                                self.logger.info('%s/%s 当前仓位成本=%.1f, 均价=%.3f, 浮盈=%.2f, 已实现盈利=%.2f（最大持有量=%s, %.1f小时）\t %s' % (
                                    self.symbol, 
                                    self.side, 
                                    sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]) * float(self.redisClient.getKey("{}_avg_tmp_{}".format(self.token, self.direction))), 
                                    float(self.redisClient.getKey("{}_avg_tmp_{}".format(self.token, self.direction))), 
                                    sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]) * (float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) - float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction)))), 
                                    float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction))),
                                    float(self.redisClient.getKey("{}_max_position_{}".format(self.token, self.direction))), 
                                    (time.time() - self.t_start) / 3600, 
                                    PublicModels.changeTime(time.time())))
                        ...
                        # 下单平仓
                        res_long = trade.open_order(self.symbol, 'BUY', [float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)][-1], price=float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), positionSide='SHORT').json()
                        self.redisClient.setKey('{}_orderId_{}'.format(self.token, res_long["orderId"]), json.dumps(res_long))
                        ...
                        nums = self.redisClient.brpopKey("{}_sell_qty".format(self.token))
                        ...
                        self.redisClient.setKey("{}_highest_price_{}".format(self.token, self.direction), 0.0)
                        self.redisClient.setKey("{}_base_price_{}".format(self.token, self.direction), self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction)))
                        self.redisClient.setKey("{}_last_buy_{}".format(self.token, self.direction), self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction)))

                        self.logger.info('%s/%s 剩余仓位成本=%.1f, 均价=%.3f, 浮盈=%.2f, 已实现盈利=%.2f（最大持有量=%s, %.1f小时）\t %s' % (
                            self.symbol,
                            self.side,
                            sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]) * self.avg,
                            float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))),
                            sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]) * (self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction)) - self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))),
                            float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction))),
                            float(self.redisClient.getKey("{}_max_position_{}".format(self.token, self.direction))),
                            (time.time() - self.t_start) / 3600,
                            PublicModels.changeTime(time.time())))
            ...
            # 如果策略为开 多 时
                ...
                self.logger.info('{}/{} U本位合约正在运行, 当前价格 {} , 已购买币种总数 {} , 已经下单总次数 {} , 锚点位置 {} \t {}'.format(
                    ...
                    sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]),
                    float(self.redisClient.llenKey("{}_sell_qty".format(self.token))),
                ...
                if int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction))) == 0:
                    ...
                    if buy_condition1:
                        ...
                        self.redisClient.setKey("{}_avg_{}".format(self.token, self.direction), self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction)))
                        self.redisClient.setKey("{}_base_price_{}".format(self.token, self.direction), self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction)))
                        self.redisClient.lpushKey("{}_sell_qty".format(self.token), self.redisClient.getKey("{}_position_size_{}".format(self.token, self.direction)))
                        self.redisClient.incrKey("{}_step_{}".format(self.token, self.direction))
                        self.redisClient.setKey("{}_last_buy_{}".format(self.token, self.direction), self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction)))
                        _win = (float(self.redisClient.getKey("{}_position_size_{}".format(self.token, self.direction))) * float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) * 4e-4) - float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction)))
                        self.redisClient.setKey("{}_win_{}".format(self.token, self.direction), _win)
                        ...
                        self.logger.info('%s/%s 当前仓位成本=%.1f, 开仓价=%.3f \t %s' % (
                            self.symbol,
                            self.side,
                            sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) * float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))),
                            float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))),
                            PublicModels.changeTime(time.time())))
                ...
                # 判断起始位大于 0, 至少开过一次仓
                    ...
                    condition = sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) / self.min_qty < self.max_add_times
                    # 判断 没有亏损 && (not 开单数量上限) && 当前价格 小于等于 最新下单价格 * (1 - 容忍爆仓率 * 持仓数量比)
                    if self.if_loss and (not condition) and float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) <= float(self.redisClient.getKey("{}_last_buy_{}".format(self.token, self.direction))) * (1 - self.add_rate * np.log(1 + int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction))))):
                        ...
                        res_short = trade.open_order(self.symbol, 'SELL', sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]), price=float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), positionSide='LONG').json()
                        self.redisClient.setKey('{}_orderId_{}_{}_{}'.format(self.token, res_short["orderId"], 'LONG', PublicModels.changeTimeNoTabs(time.time())), json.dumps(res_short))

                        if not 'orderId' in res_short:
                            self.logger.info('%s/%s 平多失败 \t %s \t %s' % (self.symbol, self.side, str(res_short), PublicModels.changeTime(time.time())))
                            continue

                        _win = (sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) * (float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) - float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction)))) * (1 - 4e-4)) + float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction)))
                        self.redisClient.setKey("{}_win_{}".format(self.token, self.direction), _win)
                        self.redisClient.setKey("{}_avg_{}".format(self.token, self.direction), 0.0)
                        self.redisClient.setKey("{}_last_buy_{}".format(self.token, self.direction), 0.0)
                        self.redisClient.setKey("{}_lowest_price_{}".format(self.token, self.direction), 100000.0)
                        ...
                        self.redisClient.setKey("{}_base_price_{}".format(self.token, self.direction), 0.0)
                        self.redisClient.setKey("{}_avg_tmp_{}".format(self.token, self.direction), 0.0)
                        self.redisClient.delKey("{}_sell_qty".format(self.token))

                    # 当前价格小于购买价格时的比例价格则进行 虚亏加仓
                    # 亏本达到 add_rate% * 持仓数量 时进行虚亏加仓, 判定值根据持仓单的数据进行上下浮动
                    elif condition and float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) <= float(self.redisClient.getKey("{}_last_buy_{}".format(self.token, self.direction))) * (1 - self.add_rate * np.log(1 + self.redisClient.setKey("{}_step_{}".format(self.token, self.direction), 0))):
                        _lowest_price = min(float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), float(self.redisClient.getKey("{}_lowest_price_{}".format(self.token, self.direction))))
                        self.redisClient.setKey("{}_lowest_price_{}".format(self.token, self.direction), _lowest_price)
                        if float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) >= float(self.redisClient.getKey("{}_lowest_price_{}".format(self.token, self.direction))) * (1 + (1 - float(self.redisClient.getKey("{}_lowest_price_{}".format(self.token, self.direction))) / float(self.redisClient.getKey("{}_last_buy_{}".format(self.token, self.direction)))) / 5):
                            self.logger.info('{}/{} 虚亏加仓 {} {}'.format(self.symbol, self.side, sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]), PublicModels.changeTime(time.time())))
                            ...
                            res_long = trade.open_order(self.symbol, 'BUY', sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]), price=float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), positionSide='LONG').json()
                            self.redisClient.setKey('{}_orderId_{}_{}_{}'.format(self.token, res_long["orderId"], 'LONG', PublicModels.changeTimeNoTabs(time.time())), json.dumps(res_long))
                            ...
                            self.redisClient.setKey("{}_avg_{}".format(self.token, self.direction), _avg)
                            self.redisClient.setKey("{}_last_buy_{}".format(self.token, self.direction), float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))))
                            self.redisClient.lpushKey("{}_sell_qty".format(self.token), sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]))
                            self.redisClient.incrKey("{}_step_{}".format(self.token, self.direction))
                            _win = ([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)][-1] * float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) * 4e-4) - float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction)))
                            self.redisClient.setKey("{}_avg_{}".format(self.token, self.direction), _win)
                            ...
                            self.logger.info('%s/%s 当前仓位成本=%.1f, 均价=%.3f, 浮亏=%.2f, 已实现盈利=%.2f（最大持有量=%s, %.1f小时）\t %s' % (
                                ...
                                sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) * float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))), 
                                float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))),
                                sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) * (float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) - float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction)))),
                                float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction))),
                                float(self.redisClient.getKey("{}_max_position_{}".format(self.token, self.direction))),
                                (time.time() - self.t_start) / 3600,
                                PublicModels.changeTime(time.time())))

                    elif (not condition) and float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) <= float(self.redisClient.getKey("{}_last_buy_{}".format(self.token, self.direction))) * (1 - self.add_rate * np.log(1 + int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction))))):
                        self.logger.info('{}/{} 重新开始下一轮 {}'.format(self.symbol, self.side, PublicModels.changeTime(time.time())))
                        ...
                        res_long = trade.open_order(self.symbol, 'SELL', sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)][-2:]), price=round(float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))) * (1 + self.min_profit), self.price_precision), positionSide='LONG').json()
                        self.redisClient.setKey('{}_orderId_{}_{}_{}'.format(self.token, res_long["orderId"], 'LONG', PublicModels.changeTimeNoTabs(time.time())), json.dumps(res_long))
                        res_long = trade.open_order(self.symbol, 'SELL', sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)][:-2]), price=round(float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))) * (1 + self.profit), self.price_precision), positionSide='LONG').json()
                        self.redisClient.setKey('{}_orderId_{}_{}_{}'.format(self.token, res_long["orderId"], 'LONG', PublicModels.changeTimeNoTabs(time.time())), json.dumps(res_long))
                    ...
                    # 当前价格如果大于 利润 profit% 或者大于 self.min_profit 即可进行盈利平多或加仓
                        ...
                        if float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) <= float(self.redisClient.getKey("{}_highest_price_{}".format(self.token, self.direction))) * (1 - (float(self.redisClient.getKey("{}_highest_price_{}".format(self.token, self.direction))) / float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))) - 1) / 5):  # 重仓情形考虑回本平一半或平xx%的仓位, 待计算, 剩下依然重仓考虑吃多少点清仓
                            ...
                            if float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) >= float(self.redisClient.getKey("{}_base_price_{}".format(self.token, self.direction))) * (1 + self.profit):
                                ...
                                # 计算开单均价
                                _avg_tmp = (float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))) * sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) / self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)[0] + float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction)))) / (sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) / [float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)][0] + 1)
                                self.redisClient.setKey("{}_avg_tmp_{}".format(self.token, self.direction), _avg_tmp)
                                ...
                                res_long = trade.open_order(self.symbol, 'BUY', self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)[0], price=float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), positionSide='LONG').json()
                                self.redisClient.setKey('{}_orderId_{}_{}_{}'.format(self.token, res_long["orderId"], 'LONG', PublicModels.changeTimeNoTabs(time.time())), json.dumps(res_long))
                                ...
                                _base_price = (1 + self.profit) * self.redisClient.getKey("{}_base_price_{}".format(self.token, self.direction))
                                self.redisClient.setKey("{}_base_price_{}".format(self.token, self.direction), _base_price)
                                self.redisClient.lpush("{}_sell_qty".format(self.token), self.redisClient.getKey("{}_position_size_{}".format(self.token, self.direction)))
                                _win = (self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)[-1] * self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction)) * 4e-4) - float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction)))
                                self.redisClient.setKey("{}_win_{}".format(self.token, self.direction), _win)
                                ...
                                self.logger.info('%s/%s 当前仓位成本=%.1f, 均价=%.3f, 浮盈=%.2f, 已实现盈利=%.2f（最大持有量=%s, %.1f小时）\t %s' % (
                                    self.symbol, 
                                    self.side, 
                                    sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) * float(self.redisClient.getKey("{}_avg_tmp_{}".format(self.token, self.direction))), 
                                    float(self.redisClient.getKey("{}_avg_tmp_{}".format(self.token, self.direction))), 
                                    sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) * (float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) - float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction)))), 
                                    float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction))),
                                    float(self.redisClient.getKey("{}_max_position_{}".format(self.token, self.direction))), 
                    ...
                    # elif int(self.redisClient.getKey("{}_step_{}".format(self.token, self.direction))) > 1 and float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))) >= self.avg * (1 + 0.003):
                        ...
                        res_short = trade.open_order(self.symbol, 'SELL', [float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)][-1], price=float(self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction))), positionSide='LONG').json()
                        self.redisClient.setKey('{}_orderId_{}_{}_{}'.format(self.token, res_short["orderId"], 'LONG', PublicModels.changeTimeNoTabs(time.time())), json.dumps(res_short))
                        ...
                        self.logger.info('%s/%s 剩余仓位成本=%.1f, 均价=%.3f, 浮盈=%.2f, 已实现盈利=%.2f（最大持有量=%s, %.1f小时）\t %s' % (
                            self.symbol,
                            self.side,
                            sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) * self.avg,
                            float(self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))),
                            sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)]) * (self.redisClient.getKey("{}_present_price_{}".format(self.token, self.direction)) - self.redisClient.getKey("{}_avg_{}".format(self.token, self.direction))),
                            float(self.redisClient.getKey("{}_win_{}".format(self.token, self.direction))),
                            float(self.redisClient.getKey("{}_max_position_{}".format(self.token, self.direction))),
                            (time.time() - self.t_start) / 3600,
                            PublicModels.changeTime(time.time())))
            ...
            _max_position = max(
                float(self.redisClient.getKey("{}_max_position_{}".format(self.token, self.direction))),
                sum([float(item) for item in self.redisClient.lrangeKey("{}_buy_qty".format(self.token), 0, -1)]),
                sum([float(item) for item in self.redisClient.lrangeKey("{}_sell_qty".format(self.token), 0, -1)])) / self.min_qty
