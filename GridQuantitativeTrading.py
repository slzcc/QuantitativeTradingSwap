...
from utils.trade_api import TradeApi
from conf.settings import *
from utils.public import *
from utils import public as PublicModels

def get_future_price(coin):
    return requests.get(url='{}/fapi/v1/ticker/price?symbol={}'.format(FUTURE_URL, coin), proxies=PROXIES_DEFAULT_DATA).json()['price']

def to_log(msg):
    with open('logs/GridQuantitativeTrading.log','a+') as f:
        f.write(msg+'\n')

class GridStrategy:
    def __init__(self, symbol, price_precision, qty_precision, mid_price, min_qty, position_times=3, size=0.5):
        self.step = 0
        self.symbol = "{}{}".format(symbol, 'USDT')
        ...
        while 1:
            ...
            while 1:
                ...
                if self.step<100 and self.present_price <= left:
                    ...
                    # print('步数=%d，现价=%s，网格区间=[%.*f,%.*f],已完成格子数=%d，浮亏格子数=%d' % (self.step, self.present_price, self.price_precision,left, self.price_precision,right,self.count,self.step*(self.step+1)//2))
                if self.count%50==0:
                    to_log('已完成网格数:%d，未完成格子数:%d\t%s'%(self.count,self.step*(self.step+1)//2, PublicModels.changeTime(time.time())))
